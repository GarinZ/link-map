    ...
    (function () {
        ...
        $.fn.position = function (options) {
            ...
            var atOffset,
                ...
                target =
                    typeof options.of === 'string' ? $(document).find(options.of) : $(options.of),
    ...
    (function (factory) {
        ...
        function FancytreeNode(parent, obj) {
            ...
            for (name in obj) {
                if (
                    !NODE_ATTR_MAP[name] &&
                    (this.tree.options.copyFunctionsToData || !_isFunction(obj[name])) &&
        ...
        FancytreeNode.prototype = /** @lends FancytreeNode# */ {
            ...
            fromDict: function (dict) {
                ...
                for (var name in dict) {
                    ...
                    } else if (!_isFunction(dict[name]) && !NONE_NODE_DATA_MAP[name]) {
			...
			*/
            ...
            scrollIntoView: function (effects, options) {
                ...
                if (isParentWindow) {
                    ...
                    topNodeY = topNode
                        ? $(topNode.span).offset().top - containerOffsetTop + scrollTop
                        : 0;
		...
		*/
            ...
            generateFormElements: function (selected, active, opts) {
                ...
                var nodeList,
                    selectedName =
                        typeof selected === 'string' ? selected : 'ft_' + this._id + '[]',
            ...
            /** @lends Fancytree_Hooks# */
            {
                ...
                nodeKeydown: function (ctx) {
                    ...
                    var matchNode,
                        ...
                        isAlnum =
                            !MODIFIERS[which] && !SPECIAL_KEYCODES[which] && !specialModifiers,
                        $target = $(event.target),
                        handled = true,
                ...
                nodeLoadChildren: function (ctx, source) {
                    ...
                    ajaxDfd.then(
                        function (data, textStatus, jqXHR) {
                            ...
                            if (
                                (source.dataType === 'json' || source.dataType === 'jsonp') &&
                                typeof data === 'string'
                            ) {
                                $.error(
                            ...
                            } else if (data && _hasProp(data, 'd') && ctx.options.enableAspx) {
                    ...
                    resultDfd
                        .done(function (data) {
                            ...
                            _assert(_isArray(children), 'expected array of children');
                ...
                nodeRemoveChild: function (ctx, childNode) {
                    ...
                    if (
                        this.activeNode &&
                        (childNode === this.activeNode || this.activeNode.isDescendantOf(childNode))
                    ) {
                        this.activeNode.setActive(false); // TODO: don't fire events
                    }
                    if (
                        this.focusNode &&
                        (childNode === this.focusNode || this.focusNode.isDescendantOf(childNode))
                    ) {
                        this.focusNode = null;
                    }
                    ...
                    if (!isRootNode) {
                        ...
                        if (node.li && (force || node.li.parentNode !== node.parent.ul)) {
                            if (node.li.parentNode === node.parent.ul) {
                                ...
                                this.debug(
                                    'Unlinking ' + node + ' (must be child of ' + node.parent + ')',
                                );
                            }
                ...
                nodeRenderTitle: function (ctx, title) {
                    ...
                    if (icon !== false) {
                        ...
                        if (typeof icon === 'string') {
                            if (TEST_IMG.test(icon)) {
                                ...
                                icon =
                                    icon.charAt(0) === '/' ? icon : (opts.imagePath || '') + icon;
                                ares.push(
                                    "<img src='" +
                                        icon +
                    ...
                    if (opts.enhanceTitle) {
                        ...
                        nodeTitle =
                            opts.enhanceTitle.call(tree, { type: 'enhanceTitle' }, ctx) || '';
                    }
                },
                /** Update element classes according to node state.
                 * @param {EventData} ctx
                ...
                nodeSetActive: function (ctx, flag, callOpts) {
                    ...
                    if (
                        flag &&
                        !noEvents &&
                        this._triggerNodeEvent('beforeActivate', node, ctx.originalEvent) === false
                    ) {
                ...
                nodeSetExpanded: function (ctx, flag, callOpts) {
                    ...
                    if ((node.expanded && flag) || (!node.expanded && !flag)) {
                        ...
                        !noEvents &&
                        this._triggerNodeEvent('beforeExpand', node, ctx.originalEvent) === false
                    ) {
                        // Callback returned false
                        return _getRejectedPromise(node, ['rejected']);
                    }
                    ...
                    _afterLoad = function (callback) {
                        ...
                        node.expanded = flag;
                        tree._callHook('treeStructureChanged', ctx, flag ? 'expand' : 'collapse');
                    ...
                    tree.$container
                        .on('focusin' + ns + ' focusout' + ns, function (event) {
                            ...
                            if (node) {
                                ...
                                if (
                                    tree.tbody &&
                                    $(event.target).parents('table.fancytree-container > thead')
                                        .length
                                ) {
                            ...
                            try {
                                ...
                                switch (event.type) {
                                    case 'click':
                                        ...
                                        return tree._triggerNodeEvent('click', ctx, event) === false
                                            ? false
                                            : tree._callHook('nodeClick', ctx);
                                    case 'dblclick':
                                        ctx.targetType = et.type;
                                        return tree._triggerNodeEvent('dblclick', ctx, event) ===
                                            false
                                            ? false
                        ...
                        if (allData && !$.isEmptyObject(allData)) {
                            ...
                            for (lowerCaseAttr in NODE_ATTR_LOWERCASE_MAP) {
                                if (_hasProp(allData, lowerCaseAttr)) {
                                    allData[NODE_ATTR_LOWERCASE_MAP[lowerCaseAttr]] =
 ...
 */
    ...
    (function (factory) {
        ...
        function applyDropEffectCallback(event, data, allowDrop) {
            ...
            if (event.type !== 'dragstart' && REQUESTED_EFFECT_ALLOWED !== data.effectAllowed) {
                tree.warn(
        ...
        function onDropEvent(event) {
            ...
            switch (event.type) {
                case 'dragenter':
                    ...
                    isSourceFtNode = $.inArray(nodeMimeType, dataTransfer.types) >= 0;
                ...
                case 'dragover':
                    ...
                    allowAutoExpand = LAST_HIT_MODE === 'over' || LAST_HIT_MODE === false;

                    if (allowAutoExpand && !node.expanded && node.hasChildren() !== false) {
                        if (!DRAG_OVER_STAMP) {
                ...
                case 'drop':
                    ...
                    if (orgSourceElem && !document.body.contains(orgSourceElem)) {
                        ...
                        if (orgSourceTree === tree) {
                            tree.debug('Drop handler removed source element: generating dragEnd.');
        ...
        $.ui.fancytree.registerExtension({
            ...
            treeInit: function (ctx) {
                ...
                if (dndOpts.preventRecursiveMoves != null) {
                    $.error('preventRecursiveMoves was renamed to preventRecursion.');
                }
                ...
                if (dndOpts.dragStart) {
                    ...
                    tree.$container.on('dragstart drag dragend', onDragEvent.bind(tree));
                }
                // Enable drop support if dragEnter() is specified:
                if (dndOpts.dragEnter) {
                    // Bind drop event handlers
                    tree.$container.on('dragenter dragover dragleave drop', onDropEvent.bind(tree));
                }
            },
        });
        // Value returned by `require('jquery.fancytree..')`
    ...
    (function (factory) {
        ...
        $.ui.fancytree._FancytreeClass.prototype.updateFilter = function () {
            if (this.enableFilter && this.lastFilterArgs && this.options.filter.autoApply) {
            ...
            nodeRenderStatus: function (ctx) {
                ...
                if (span) {
                    ...
                    if ((node.parent && node.parent.radiogroup) || checkbox === 'radio') {
                        ...
                        setIcon(node, span, 'fancytree-checkbox fancytree-radio', opts, icon);
                    } else {
                        // eslint-disable-next-line no-nested-ternary
                        icon = node.selected
                            ? 'checkboxSelected'
    ...
    (function (factory) {
        if (typeof define === 'function' && define.amd) {
            // AMD. Register as an anonymous module.
            define(['jquery', './jquery.fancytree', './jquery.fancytree.table'], factory);
        } else if (typeof module === 'object' && module.exports) {
            // Node/CommonJS
        ...
        $.ui.fancytree.registerExtension({
            ...
            treeInit: function (ctx) {
                ...
                if ($.isPlainObject(instOpts.store)) {
                    ...
                    local.store =
                        instOpts.store === 'local' ? localStorageStore : sessionStorageStore;
                                ...
                                for (i = 0; i < keyList.length; i++) {
                                    ...
                                    if (node) {
                                        if (
                                            ...
                                            (instOpts.overrideSource && node.selected === false)
                                        ) {
                                            //									node.setSelected();
                                            node.selected = true;
                                            node.renderStatus();
    ...
    (function (factory) {
        ...
        $.ui.fancytree.registerExtension({
            ...
            treeInit: function (ctx) {
                ...
                // Prepare row view.templates:
                // Determine column count from table header if any
                tree.columnCount = $('thead >tr', $table).last().find('>th', $table).length;
                // Read TR view.templates from tbody if any
                ...
                if (opts.aria) {
                    tree.$container.attr('role', 'treegrid').attr('aria-readonly', true);
                }
            },
            nodeRemoveChildMarkup: function (ctx) {
                var node = ctx.node;
            ...
            nodeRenderTitle: function (ctx, title) {
                ...
                if (isStatusNode) {
                    if (opts.renderStatusColumns) {
                        ...
                        opts.renderStatusColumns.call(tree, { type: 'renderStatusColumns' }, ctx);
    ...
    (function (factory) {
        ...
        $.ui.fancytree.registerExtension({
            ...
            treeCreate: function (ctx) {
                ...
                this._local.lineOfs =
                    (1 + (ctx.options.checkbox ? 1 : 0) + (ctx.options.icon === false ? 0 : 1)) *
